...
import type { SeedFormState } from '@/lib/seed/state';
...
import { triggerSaveCodeNow } from '@/lib/seed/swh';
...
export function AddPackageModal({ open, onClose }: Props) {
  ...
  const [archiveInSwh, setArchiveInSwh] = useState(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Fire Save Code Now in parallel with YAML generation if the user
      // opted in. The result lands in the seed YAML so the backfill
      // cron knows this entry is awaiting archival. Failures here are
      // non-fatal — the cron retries via /visit/latest/ regardless.
      const swh = archiveInSwh ? await triggerSaveCodeNow(repoCanonicalUrl) : undefined;

      const yaml = toSeedYaml(result.state, swh ? { submittedAt: swh.submittedAt } : undefined);
    ...
    >
      ...
      >
        <header className="flex items-center justify-between border-b border-border px-6 py-4">
          <h2 id="add-package-title" className="text-xl font-semibold text-text">
            Add a package
          </h2>
        ...
        <div className="flex flex-col gap-5 p-6">
          {status.kind === 'submitted' ? (
            <SuccessPanel onClose={handleClose} archived={archiveInSwh} />
          ) : (
            <UrlEntry
              repoUrl={repoUrl}
              setRepoUrl={setRepoUrl}
              archiveInSwh={archiveInSwh}
              setArchiveInSwh={setArchiveInSwh}
              status={status}
...
function SuccessPanel({ onClose, archived }: { onClose: () => void; archived: boolean }) {
  return (
    <div className="flex flex-col gap-4">
      <Banner>
        <strong className="font-semibold">Pull request opened on GitHub.</strong> Review the
        prefilled file and click <em>“Commit new file → Propose changes”</em>. GitHub will fork the
        repo for you if needed. Our CI validates the schema automatically.
      </Banner>
      {archived && (
        <Banner>
          <strong className="font-semibold">Archival requested at Software Heritage.</strong> SWH
          will ingest your repo over the next few hours. Once it does, our backfill cron writes the
          SWHID into your seed entry — no action needed from you.
        </Banner>
      )}
...
interface UrlEntryProps {
  ...
  setArchiveInSwh: (b: boolean) => void;
  status: Status;
  onSubmit: () => void;
}
  ...
  return (
    <>
      <p className="text-sm text-text-muted">
        ...
        <code className="rounded bg-code-bg px-1 py-0.5 font-mono text-[0.85em]">CITATION.cff</code>{' '}
        and{' '}
        <code className="rounded bg-code-bg px-1 py-0.5 font-mono text-[0.85em]">
          codemeta.json
        </code>
        , build a Citey citation, and open a pull request for you. Make sure those files exist and
        are well-formed first.
      </p>

      <div className="flex flex-col gap-2">
        <label htmlFor="add-repo-url" className="text-sm font-semibold text-text">
          GitHub repo URL
        </label>
        <div className="flex flex-wrap items-stretch gap-2">
          <input
            ...
            autoFocus
          />
          <Button variant="primary" onClick={onSubmit} disabled={loading || !repoUrl.trim()}>
      ...
      <label className="flex items-start gap-2 text-sm text-text">
        <input
          type="checkbox"
          checked={archiveInSwh}
          onChange={(e) => setArchiveInSwh(e.target.checked)}
          className="mt-0.5"
        />
        <span>
          Archive my code in{' '}
          ...
          and include a SWHID in my citation. Free, no account needed, gives your software a
          permanent ISO 18670 identifier.
        </span>
      </label>

      {status.kind === 'error' && (
        <Banner>
          <strong className="font-semibold">Couldn&rsquo;t submit.</strong> {status.message}
        </Banner>
      )}
